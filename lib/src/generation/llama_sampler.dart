...
/// Grammar sampler utilities
class GrammarSamplerUtils {
  /// Create a JSON grammar sampler
  static Pointer<llama_sampler> createJsonGrammar(llama_cpp llamaCpp, Pointer<llama_vocab> vocab) {
    final jsonGrammar = '''
root ::= object
...
''';
    ...
    try {
      return llamaCpp.llama_sampler_init_grammar(vocab, grammarPtr.cast<Char>(), rootPtr.cast<Char>());
    } finally {
  ...
  /// Create a custom grammar sampler
  static Pointer<llama_sampler> createCustomGrammar(
    llama_cpp llamaCpp,
    Pointer<llama_vocab> vocab,
    String grammar,
    String rootRule,
  ) {
    final grammarPtr = grammar.toNativeUtf8();
    final rootPtr = rootRule.toNativeUtf8();
  ...
  static const Map<String, String> commonGrammars = {
    'json': '''
root ::= object
object ::= "{" ws ( string ":" ws value ( "," ws string ":" ws value )* )? ws "}"
array ::= "[" ws ( value ( "," ws value )* )? ws "]"
